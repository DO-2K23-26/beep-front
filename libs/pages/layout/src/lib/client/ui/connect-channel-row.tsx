...
export function ConnectedChannelRow({ onLeave, channelName, serverName, connectionState }: ConnectedChannelRowProps) {
  return (
    <div className="w-full pb-3 border-b-2 border-violet-400 flex flex-row items-center justify-between">
      <div className='flex flex-col gap-2' >
        ...
        {["connecting", "new"].includes(connectionState) &&
          <Badge type={BadgeType.WARNING} title={connectionState} className="!text-slate-900" />
        }
        {["connected"].includes(connectionState) &&
          <Badge type={BadgeType.ONLINE} title={connectionState} className="!text-slate-900" />
        }
      </div>
