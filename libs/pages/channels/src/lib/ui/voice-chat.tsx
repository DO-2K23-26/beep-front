...
export function VoiceChat() {
  ...
  const gridSize = videoCount <= 2 ? "minmax(300px, 1fr)" : videoCount <= 4 ? "minmax(200px, 1fr)" : "minmax(150px, 1fr)";
      ...
      {expandedStream ? (
        <>
          <div className="w-full h-2/3 relative border rounded-xl overflow-hidden mb-4 group">
            <video
              ref={(video) => video && (video.srcObject = expandedStream?.stream)}
              autoPlay
              playsInline
        ...
        <div
          ...
          style={{
            ...
            gridTemplateRows: `repeat(auto-fit, ${gridSize})`,
            gridAutoColumns: gridSize,
          }}
        >
          {paginatedStreams.map(({ username, id, stream }) => (
            <div
              key={id}
              className="relative w-full h-full border rounded-xl overflow-hidden group"
            >
              <video
                ref={(video) => video && (video.srcObject = stream)}
                autoPlay
